...
import mxnet as mx
import numpy as np
from lib.bbox.bbox_transform import bbox_overlaps, bbox_transform, remove_repetition
from lib.bbox.bbox_regression import expand_bbox_regression_targets
from lib.mask.mask_transform import intersect_box_mask
import pickle
...
class ProposalAnnotatorOperator(mx.operator.CustomOp):
    ...
    def forward(self, is_train, req, in_data, out_data, aux):
        ...
        rois, labels, bbox_targets, bbox_weights, mask_reg_targets = \
            self.sample_rois(all_rois, fg_rois_per_image, rois_per_image,
                             self._num_classes,
    ...
    def sample_rois(self, rois, fg_rois_per_image, rois_per_image,
                    num_classes, cfg,
        ...
        for idx, obj in enumerate(fg_indexes):
            ...
            # print('~~~~~~', obj, gt_assignment[obj], gt_masks.shape, gt_assignment.shape)
...
class ProposalAnnotatorProp(mx.operator.CustomOpProp):
    def __init__(self, num_classes, mask_size, binary_thresh, batch_images,
                 batch_rois, cfg, fg_fraction='0.25'):
