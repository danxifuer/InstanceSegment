...
from fcis.operator_py.proposal import *
from fcis.operator_py.proposal_annotator import *
from fcis.operator_py.box_parser import *
from fcis.operator_py.box_annotator_ohem import *
...
class resnet_v1_101_fcis(Symbol):
    ...
    def get_symbol(self, cfg, is_train=True):
        ...
        if is_train:
            ...
            temp_cfg_file = 'proposal_annotator.pkl'
            pickle.dump(cfg, open(temp_cfg_file, 'wb'))
            group = mx.sym.Custom(rois=rois, gt_boxes=gt_boxes_reshape,
                                  gt_masks=gt_masks,
                                  op_type='proposal_annotator',
                                  num_classes=num_reg_classes,
                                  mask_size=cfg.MASK_SIZE,
                                  binary_thresh=cfg.TRAIN.BINARY_THRESH,
        ...
        psroipool_cls_seg = mx.contrib.sym.PSROIPooling(name='psroipool_cls_seg', data=fcis_cls_seg, rois=rois,
                                                        group_size=7, pooled_size=21, output_dim=num_classes * 2,
                                                        spatial_scale=0.0625)
        psroipool_bbox_pred = mx.contrib.sym.PSROIPooling(name='psroipool_bbox', data=fcis_bbox, rois=rois,
                                                          group_size=7, pooled_size=21, output_dim=num_reg_classes * 4,
                                                          spatial_scale=0.0625)
        if is_train:
            # classification path
            psroipool_cls = mx.contrib.sym.ChannelOperator(name='psroipool_cls', data=psroipool_cls_seg,
                                                           group=num_classes, op_type='Group_Max')
            ...
            seg_pred = mx.contrib.sym.ChannelOperator(name='seg_pred', data=psroipool_cls_seg, pick_idx=label_seg,
            ...
            seg_softmax = mx.contrib.sym.ChannelOperator(name='seg_softmax', data=psroipool_cls_seg, group=num_classes,
                                                         op_type='Group_Softmax')
            seg_pred = mx.contrib.sym.ChannelOperator(name='seg_pred', data=seg_softmax, pick_idx=score_seg,
                                                      group=num_classes, op_type='Group_Pick', pick_type='Score_Pick')
            # bbox regression path
            bbox_pred = mx.sym.Pooling(name='bbox_pred', data=psroipool_bbox_pred, pool_type='avg', global_pool=True,
                                       kernel=(21, 21))
            bbox_pred = mx.sym.Reshape(name='bbox_pred_reshape', data=bbox_pred, shape=(-1, 4 * num_reg_classes))
        ...
        if is_train:
            if cfg.TRAIN.ENABLE_OHEM:
                ...
                cls_prob = mx.sym.SoftmaxOutput(name='cls_prob', data=cls_score, label=label, normalization='valid',
                                                use_ignore=True, ignore_label=-1, grad_scale=cfg.TRAIN.LOSS_WEIGHT[0])
                seg_prob = mx.sym.SoftmaxOutput(name='seg_prob', data=seg_pred, label=mask_reg_targets,
                                                multi_output=True,
                                                normalization='null', use_ignore=True, ignore_label=-1,
                                                grad_scale=cfg.TRAIN.LOSS_WEIGHT[1] / cfg.TRAIN.BATCH_ROIS)
                bbox_loss_t = bbox_weight * mx.sym.smooth_l1(name='bbox_loss_t', scalar=1.0,
                                                             data=(bbox_pred - bbox_target))
                bbox_loss = mx.sym.MakeLoss(name='bbox_loss', data=bbox_loss_t,
                                            grad_scale=cfg.TRAIN.LOSS_WEIGHT[2] / cfg.TRAIN.BATCH_ROIS)
                rcnn_label = label
            ...
            group = mx.sym.Group(
                [rpn_cls_prob, rpn_bbox_loss, cls_prob, bbox_loss, seg_prob, mx.sym.BlockGrad(mask_reg_targets),
            ...
            if cfg.TEST.ITER == 2:
                rois_iter2 = mx.sym.Custom(bottom_rois=rois, bbox_delta=bbox_pred, im_info=im_info, cls_prob=cls_prob,
                                           ...
                                           bbox_means=tuple(cfg.TRAIN.BBOX_MEANS), bbox_stds=tuple(cfg.TRAIN.BBOX_STDS),
                                           op_type='BoxParser')
                # rois = mx.sym.Concat(*[rois, rois_iter2], dim=0, name='rois')
                psroipool_cls_seg_iter2 = mx.contrib.sym.PSROIPooling(name='psroipool_cls_seg', data=fcis_cls_seg,
                                                                      rois=rois_iter2,
                                                                      group_size=7, pooled_size=21,
                                                                      output_dim=num_classes * 2, spatial_scale=0.0625)
                psroipool_bbox_pred_iter2 = mx.contrib.sym.PSROIPooling(name='psroipool_bbox', data=fcis_bbox,
                                                                        rois=rois_iter2,
                                                                        group_size=7, pooled_size=21,
                                                                        output_dim=num_reg_classes * 4,
                                                                        spatial_scale=0.0625)

                # classification path
                psroipool_cls_iter2 = mx.contrib.sym.ChannelOperator(name='psroipool_cls', data=psroipool_cls_seg_iter2,
                                                                     group=num_classes,
                                                                     op_type='Group_Max')
                cls_score_iter2 = mx.sym.Pooling(name='cls_score', data=psroipool_cls_iter2, pool_type='avg',
                                                 global_pool=True, kernel=(21, 21), stride=(21, 21))

                cls_score_iter2 = mx.sym.Reshape(name='cls_score_reshape', data=cls_score_iter2,
                                                 shape=(-1, num_classes))
                ...
                seg_softmax_iter2 = mx.contrib.sym.ChannelOperator(name='seg_softmax', data=psroipool_cls_seg_iter2,
                                                                   group=num_classes, op_type='Group_Softmax')
                seg_pred_iter2 = mx.contrib.sym.ChannelOperator(name='seg_pred', data=seg_softmax_iter2,
                                                                pick_idx=score_seg_iter2, group=num_classes,
                                                                op_type='Group_Pick', pick_type='Score_Pick')
                # bbox regression path
                bbox_pred_iter2 = mx.sym.Pooling(name='bbox_pred', data=psroipool_bbox_pred_iter2, pool_type='avg',
                                                 global_pool=True, kernel=(21, 21), stride=(21, 21))
                bbox_pred_iter2 = mx.sym.Reshape(name='bbox_pred_reshape', data=bbox_pred_iter2,
                                                 shape=(-1, 4 * num_reg_classes))
