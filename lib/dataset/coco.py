...
from lib.utils.mask_coco2voc import mask_coco2voc
from lib.utils.mask_voc2coco import mask_voc2coco
...
def generate_cache_seg_inst_kernel(annWithObjs):
    ...
    if not gt_mask_file:
        return
    gt_mask_flip_file = os.path.join(os.path.splitext(gt_mask_file)[0] + '_flip.pkl')
    ...
    if not os.path.exists(gt_mask_flip_file):
        cPickle.dump(gt_mask[:, :, ::-1].astype('bool'),
                     open(gt_mask_flip_file, 'wb'))
