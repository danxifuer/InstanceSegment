...
from lib.dataset.pascal_voc_eval import voc_eval, voc_eval_sds
from lib.dataset.ds_utils import unique_boxes, filter_small_boxes


class NucleiDataset(IMDB):
    def __init__(self, image_set, root_path, devkit_path, result_path=None, mask_size=-1, binary_thresh=None):
        ...
        super(NucleiDataset, self).__init__('voc_' + year, image_set, root_path,
    ...
    def image_path_from_index(self, index):
        ...
        if not os.path.exists(image_file):
            image_file = os.path.join(self.data_path, 'img', index + '.png')
    ...
    def mask_path_from_index(self, index, gt_mask):
        ...
        gt_mask_file = os.path.join(cache_file, index + '.pkl')
        if not os.path.exists(gt_mask_file):
            cPickle.dump(gt_mask.astype('bool'), open(gt_mask_file, 'wb'))
        # cache flip gt_masks
        gt_mask_flip_file = os.path.join(cache_file, index + '_flip.pkl')
        if not os.path.exists(gt_mask_flip_file):
            cPickle.dump(gt_mask[:, :, ::-1].astype('bool'), open(gt_mask_flip_file, 'wb'))
        return gt_mask_file
    ...
    def load_sbd_mask_annotations(self, index):
        ...
        seg_obj_name = os.path.join(self.data_path, 'inst', index + '.npy')
        seg_obj_data = np.load(seg_obj_name)
